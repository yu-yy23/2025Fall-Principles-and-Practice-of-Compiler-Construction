...
class Riscv:
    ...
    class NativeAddi(BackendInstr):
        def __init__(self, dst: Reg, src: Reg, imm: int) -> None:
            super().__init__(InstrKind.SEQ, [], [], None)
            self.dst = dst
            self.src = src
            self.imm = imm
        ...
        def __str__(self) -> str:
            assert -2048 <= self.imm <= 2047
            return "addi " + Riscv.FMT3.format(
                str(self.dst), str(self.src), str(self.imm)
            )

    class Call(BackendInstr):
        def __init__(self, funcLabel: str, dst: Temp, args: list[Temp]) -> None:
            super().__init__(InstrKind.SEQ, [dst] if dst is not None else [], args, funcLabel)
            self.funcLabel = funcLabel
        
        def __str__(self) -> str:
            return "call " + str(self.funcLabel)

    class Parameter(BackendInstr):
        def __init__(self, arg: Temp) -> None:
            super().__init__(InstrKind.SEQ, [], [arg], None)
            self.arg = arg
    ...
    class StoreData(BackendInstr):
        def __init__(self, src: Temp, addr: Temp, offset: int) -> None:
    ...
    class Alloc(BackendInstr):
        ...
        def __str__(self) -> str:
            return "alloc " + Riscv.FMT2.format(str(self.dsts[0]), str(self.size))
