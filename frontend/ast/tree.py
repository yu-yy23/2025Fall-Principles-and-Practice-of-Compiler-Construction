...
class Program(ListNode["Function"]):
    ...
    def __init__(self, *children: Function | Declaration) -> None:
    ...
    def functions_list(self) -> list[Function]:
        return [func for func in self if isinstance(func, Function)]

    def funcs_and_decls(self) -> list[Union[Function, Declaration]]:
        return [item for item in self]

    def global_decls(self) -> list[Declaration]:
        return [decl for decl in self if isinstance(decl, Declaration)]

    def getRedifinedFuncs(self) -> str:
        funcNames = set()
        for func in self:
            if func.ident.value in funcNames:
                return func.ident.value
            funcNames.add(func.ident.value)
        return None
...
class Function(Node):
    ...
    def __init__(
        ...
        ret_t: TypeLiteral,
        ident: Identifier,
        params: Union[ParameterList, NullType],
        body: Block,
    ) -> None:
...
class ParameterList(ListNode["Parameter"]):
    """
    AST node that represents a list of function parameters.
    """
...
class Declaration(Node):
    ...
    def __init__(
        ...
        var_t: TypeLiteral,
        ident: Identifier,
        init_exprs: Optional[list[Expression]] = None,
    ) -> None:
        super().__init__("declaration")
        self.var_t = var_t
        self.ident = ident
        self.init_exprs = init_exprs or NULL

    def __getitem__(self, key: int) -> Node:
        return (self.var_t, self.ident, self.init_exprs)[key]

    def __len__(self) -> int:
        return 3

    def __str__(self) -> str:
        return "declaration({}, {}, {})".format(
            self.var_t,
            self.ident,
            self.init_exprs,
        )
...
class Assignment(Binary):
    ...
    def __init__(self, lhs: Identifier | IndexExpression, rhs: Expression) -> None:
...
class Call(Expression):
    ...
    """
    def __init__(self, ident: str, arguments: Union["ArgumentList", "NullType"]):
        super().__init__("call")
        self.ident = ident
...
class ArgumentList(ListNode[Expression]):
    """
    AST node that represents a list of function call arguments.
    """

    def __init__(self, *children: Expression) -> None:
        super().__init__("argument_list", list(children))

    def accept(self, v: Visitor[T, U], ctx: T):
        return v.visitArgumentList(self, ctx)


class IndexExpression(Expression):
    """
    AST node of array index expression.
    """
...
class TArray(TypeLiteral):
    """
    AST node of array type like `int[10]`.
    """

    def __init__(self, base: TypeLiteral, size: Expression) -> None:
        super().__init__("type_array", ArrayType(base.type, size.value))
        self.base = base
        self.size = size
