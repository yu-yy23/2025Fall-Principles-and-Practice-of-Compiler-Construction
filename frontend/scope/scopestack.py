...
class ScopeStack:
    ...
    def lookupInFuncScope(self, name: str) -> Optional[Symbol]:
        symbol = self.top().lookup(name)
        if symbol is not None:
            return symbol
        if len(self.scopes) >= 2 and self.scopes[-2].kind == ScopeKind.FUNC_DECL:
            symbol = self.scopes[-2].lookup(name)
            if symbol is not None:
                return symbol
        return None
