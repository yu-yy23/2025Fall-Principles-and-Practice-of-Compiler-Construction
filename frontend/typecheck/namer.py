...
class Namer(Visitor[ScopeStack, None]):
    ...
    def visitProgram(self, program: Program, ctx: ScopeStack) -> None:
        ...
        for decl in program.global_decls():
            ...
            varSymbol = VarSymbol(decl.ident.value, decl.var_t.type, isGlobal=True)
    ...
    def visitDeclaration(self, decl: Declaration, ctx: ScopeStack) -> None:
        ...
        if isinstance(decl.var_t.type, ArrayType):
            arrayType = decl.var_t.type
    ...
    def visitIndexExpression(self, expr: IndexExpression, ctx: ScopeStack) -> None:
        ...
        while isinstance(expr_base, IndexExpression):
            expr_base.index.accept(self, ctx)
            index_dim += 1
            expr_base = expr_base.base

        baseSymbol = ctx.lookup(expr_base.value)
        if baseSymbol is None:
            raise DecafUndefinedVarError(expr_base.value)
        if not isinstance(baseSymbol.type, ArrayType):
            raise DecafBadIndexError()

        if isinstance(expr_base, Identifier):
            expr_base.setattr("symbol", baseSymbol)
        expr.setattr("symbol", baseSymbol)
