...
from utils.tac.tacglobaldecl import TACGlobalDecl
...
class TACFuncEmitter:
    ...
    def emitLoadSymbol(self, symbol: VarSymbol) -> Temp:
        temp = self.freshTemp()
        self.func.add(LoadSymbol(temp, symbol))
        return temp

    def emitLoadData(self, dst: Temp, addr: Temp, ofs: int) -> Temp:
        self.func.add(LoadData(dst, addr, ofs))

    def emitStoreData(self, src: Temp, addr: Temp, ofs: int) -> None:
        self.func.add(StoreData(src, addr, ofs))

    def emitAlloc(self, dst: Temp, size: int) -> None:
    ...
    def emitCall(self, label: str, args: list[Temp]) -> None:
        temp = self.freshTemp()
...
class TACGen():
    ...
    def transform(self, program: Program) -> TACProg:
        ...
        for decl in program.global_decls():
            name = decl.getattr("symbol").name
            init_vals = [decl.init_exprs[i].value for i in range(len(decl.init_exprs))] if decl.init_exprs is not NULL else None
            size = decl.getattr("symbol").type.size
            type_name = str(decl.getattr("symbol").type)
            globalDecls.append(TACGlobalDecl(name, init_vals, size, type_name))
        ...
        if self.addFillNFunction:
            emitter = TACFuncEmitter(FuncLabel("fill_n"), 3, labelManager)
            arrTemp = emitter.freshTemp()
            valTemp = emitter.freshTemp()
            nTemp = emitter.freshTemp()
            addrAddTemp = emitter.emitLoad(4)
            nSubTemp = emitter.emitLoad(-1)
            emitter.emitParameter(arrTemp)
            emitter.emitParameter(valTemp)
            emitter.emitParameter(nTemp)

            beginLabel = emitter.freshLabel()
            loopLabel = emitter.freshLabel()
            breakLabel = emitter.freshLabel()
            emitter.openLoop(breakLabel, loopLabel)

            emitter.emitBranch(beginLabel)

            emitter.emitLabel(loopLabel)
            emitter.emitStoreData(valTemp, arrTemp, 0)
            emitter.emitBinarySelf(tacop.TacBinaryOp.ADD, arrTemp, addrAddTemp)
            emitter.emitBinarySelf(tacop.TacBinaryOp.ADD, nTemp, nSubTemp)

            emitter.emitLabel(beginLabel)
            emitter.emitCondBranch(
                tacop.CondBranchOp.BEQ, nTemp, breakLabel
            )

            emitter.emitBranch(loopLabel)
            emitter.emitLabel(breakLabel)
            emitter.emitReturn(emitter.emitLoad(0))
            tacFuncs.append(emitter.emitEnd())

        return TACProg(tacFuncs, globalDecls)
    ...
    def visitIdentifier(self, ident: Identifier, mv: TACFuncEmitter) -> None:
        ...
        if symbol.isGlobalVar():
            addrTemp = mv.emitLoadSymbol(symbol)
            if isinstance(symbol.type, ArrayType):
                symbol.temp = addrTemp
            else:
                symbol.temp = mv.freshTemp()
                mv.emitLoadData(symbol.temp, addrTemp, 0)
    ...
    def visitDeclaration(self, decl: Declaration, mv: TACFuncEmitter) -> None:
        ...
        symbol.temp = mv.freshTemp()
        if isinstance(symbol.type, ArrayType):
            mv.emitAlloc(symbol.temp, symbol.type.size)
        if decl.init_exprs is not NULL:
            if isinstance(symbol.type, ArrayType):
                self.addFillNFunction = True
                arrArgTemp = mv.freshTemp()
                mv.emitAssignment(arrArgTemp, symbol.temp)
                mv.emitCall("fill_n", [arrArgTemp, mv.emitLoad(0), mv.emitLoad(symbol.type.size // 4)])
            for expr in decl.init_exprs:
                expr.accept(self, mv)
            if isinstance(symbol.type, ArrayType):
                offset = 0
                for expr in decl.init_exprs:
                    mv.emitStoreData(expr.getattr("val"), symbol.temp, offset)
                    offset += 4
            else:
                mv.emitAssignment(symbol.temp, decl.init_exprs[0].getattr("val"))
    ...
    def visitAssignment(self, expr: Assignment, mv: TACFuncEmitter) -> None:
        ...
        offset = 0
        if lhsSymbol.isGlobalVar():
            lhsSymbol.temp = mv.emitLoadSymbol(lhsSymbol)
            addrTemp = lhsSymbol.temp
        if isinstance(expr.lhs, IndexExpression):
            indexes = []
            sizes = []

            expr_base = expr.lhs
            while isinstance(expr_base, IndexExpression):
                expr_base.index.accept(self, mv)
                indexes.append(expr_base.index.getattr("val"))
                expr_base = expr_base.base
            indexes.reverse()

            baseSymbol = expr.lhs.getattr("symbol")
            
            arrayType = baseSymbol.type
            while isinstance(arrayType, ArrayType):
                sizes.append(arrayType.length)
                arrayType = arrayType.base

            offsetTemp = mv.emitLoad(0)
            for i in range(len(indexes)):
                mv.emitBinarySelf(tacop.TacBinaryOp.MUL, offsetTemp, mv.emitLoad(sizes[i]))
                mv.emitBinarySelf(tacop.TacBinaryOp.ADD, offsetTemp, indexes[i])
            for i in range(len(indexes), len(sizes)):
                mv.emitBinarySelf(tacop.TacBinaryOp.MUL, offsetTemp, mv.emitLoad(sizes[i]))
            mv.emitBinarySelf(tacop.TacBinaryOp.MUL, offsetTemp, mv.emitLoad(arrayType.size))
            if baseSymbol.isGlobalVar():
                baseTemp = mv.emitLoadSymbol(baseSymbol)
            else:
                baseTemp = baseSymbol.temp
            addrTemp = mv.emitBinary(tacop.TacBinaryOp.ADD, baseTemp, offsetTemp)
        if isinstance(expr.lhs, IndexExpression) or lhsSymbol.isGlobalVar():
            mv.emitStoreData(expr.rhs.getattr("val"), addrTemp, offset)
        else:
    ...
    def visitIndexExpression(self, expr: IndexExpression, mv: TACFuncEmitter) -> None:
        indexes = []
        sizes = []
        ...
        baseSymbol = expr.getattr("symbol")
        ...
        exprTemp = mv.freshTemp()
        expr.setattr("val", exprTemp)
        mv.emitLoadData(
            exprTemp, addrTemp, 0
        )
