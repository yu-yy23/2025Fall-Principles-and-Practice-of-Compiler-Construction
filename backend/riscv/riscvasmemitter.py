...
class RiscvAsmEmitter():
    def __init__(
        ...
        globalDecls: list[TACGlobalDecl],
    ):
        ...
        # emit the global declarations at
        self.printer.printComment("global declaration")
        self.printer.println(".data")
        for decl in self.globalDecls:
            if (decl.type == "int" and decl.init_vals is not None and decl.init_vals[0] != 0) or (decl.type != "int" and decl.init_vals is not None):
                self.printer.println(f".globl {decl.name}")
                self.printer.println(f"{decl.name}:")
                for val in decl.init_vals:
                    self.printer.println(f"    .word {val}")
        self.printer.println("")

        self.printer.println(".bss")
        for decl in self.globalDecls:
            if (decl.type != "int" and decl.init_vals is None) or (decl.type == "int" and (decl.init_vals is None or decl.init_vals[0] == 0)):
                self.printer.println(f".globl {decl.name}")
                self.printer.println(f"{decl.name}:")
                self.printer.println(f"    .space {decl.size}")
        self.printer.println("")
    ...
    class RiscvInstrSelector(TACVisitor):
        ...
        def visitLoadSymbol(self, instr: LoadSymbol) -> None:
            self.seq.append(Riscv.LoadSymbol(instr.dst, instr.symbol.name))

        def visitLoadData(self, instr: LoadData) -> None:
            self.seq.append(Riscv.LoadData(instr.dst, instr.addr, instr.offset))

        def visitStoreData(self, instr: StoreData) -> None:
            self.seq.append(Riscv.StoreData(instr.src, instr.addr, instr.offset))
        ...
        def visitCall(self, instr: Call) -> None:
            self.seq.append(Riscv.Call(instr.funcLabel, instr.ret, instr.args))

        def visitParameter(self, instr: Parameter) -> None:
            self.seq.append(Riscv.Parameter(instr.arg))
        # in step11, you need to think about how to store the array 
        def visitAlloc(self, instr: Alloc) -> None:
            # self.seq.append(Riscv.SPAdd(-instr.size))
...
class RiscvSubroutineEmitter():
    def __init__(self, emitter: RiscvAsmEmitter, info: SubroutineInfo) -> None:
        ...
        # self.containFuncCall = False

        for r in Riscv.CalleeSaved:
            r.used = False

        for r in emitter.allocatableRegs:
            r.used = False
    ...
    def emitAsm(self, instr: BackendInstr):
        if isinstance(instr, Riscv.Alloc):
            self.buf.append(Riscv.NativeAddi(instr.dsts[0], Riscv.SP, self.nextLocalOffset))
